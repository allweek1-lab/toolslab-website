...
test("privacy policy is English-primary, Korean-secondary, and names actual processors", async () => {
  ...
  assert.match(privacy, /owner-specific deduplicated queue rotates one reviewed term for each of ten categories/);
  assert.match(privacy, /newly appeared or gained search position are labeled Rising/);
  assert.match(privacy, /explicitly labeled Popular now and are not represented as rising/);
  assert.match(privacy, /Each search is limited to 25 text posts/);
  assert.match(privacy, /displays temporary individual text-post cards and opens the exact original permalink/);
  assert.match(privacy, /owner-, category-, language-, and purpose-bound AES-256-GCM ciphertext/);
  assert.match(privacy, /stale after 75 minutes and is deleted no later than two hours/);
  ...
  assert.match(privacy, /사용자 전용 중복 방지 queue가 10개 분야마다 검토 검색어 1개를 순환 선택/);
  assert.match(privacy, /새로 나타났거나 검색 위치가 상승한 개별 게시물만 급상승으로 표시/);
  assert.match(privacy, /현재 인기 게시물로 명확히 구분/);
  assert.match(privacy, /개별 텍스트 게시물 카드를 일시 표시하고 Threads의 정확한 원문 permalink/);
  assert.match(privacy, /준비 순위는 약 60분마다 분산 갱신/);
  assert.match(privacy, /snapshot은 75분 후 오래된 상태가 되고 관찰 후 최대 2시간 안에 삭제/);
...
test("support page documents the connected-user Method 2 flow and reachable help paths", async () => {
  ...
  assert.match(support, /View prepared Threads rankings/);
  assert.match(support, /Enable automatic rankings for 10 categories/);
  assert.match(support, /owner-specific queue rotates one reviewed term per category/);
  assert.match(support, /exact original post on Threads/);
  assert.match(support, /does not fall back to an operator token/);
  assert.match(support, /사용자 전용 queue가 분야마다 검토 검색어 1개를 순환 선택/);
  assert.match(support, /Threads의 정확한 원문 게시물/);
  assert.match(support, /운영자 토큰/);
  assert.match(support, /급상승 결과가 비어 있으면 두 RECENT 관찰 사이/);
  assert.match(support, /refresh about every 60 minutes, become stale after 75 minutes/);
  assert.match(support, /관찰 후 최대 2시간 안에 삭제/);
...
test("data deletion page covers in-app, local-only, and no-access deletion paths", async () => {
  ...
  assert.match(deletion, /encrypted Threads access token, app-scoped account binding, automatic-ranking preference, pending jobs/);
  assert.match(deletion, /lifecycle receipts expire within 24 hours/);
  assert.match(deletion, /Automatic baselines and completed snapshots are owner-bound AES-256-GCM ciphertext/);
  assert.match(deletion, /stale after 75 minutes, and are deleted no later than two hours/);
  assert.match(deletion, /displays safety-filtered post text, usernames, publication time, and exact official permalinks/);
  assert.match(deletion, /암호화 Threads 액세스 토큰/);
  assert.match(deletion, /자동 baseline과 완성 snapshot은 사용자별 AES-256-GCM 암호문/);
  assert.match(deletion, /75분 후 오래된 상태가 되며 관찰 후 최대 2시간 안에 삭제/);
...
test("terms identify the service as unofficial and its scores as internal", async () => {
  ...
  assert.match(terms, /같은 RECENT 검색을 61초 이상 간격으로 비교/);
  assert.match(terms, /현재 인기 게시물로 명확히 구분/);
  assert.match(terms, /약 60분마다 분산 갱신/);
  assert.match(terms, /75분 후 오래된 상태가 되며 관찰 후 최대 2시간 안에 삭제/);
