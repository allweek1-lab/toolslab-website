...
export default function Inquiry() {
  const [service, setService] = useState("49,000원 진단·작동 샘플");
  const [task, setTask] = useState("");
  const [files, setFiles] = useState("");
  const [deadline, setDeadline] = useState("");
  const [notice, setNotice] = useState("");
  const body = `안녕하세요. 업무 자동화를 문의합니다.\n\n문의 유형: ${service}\n반복하는 업무: ${task}\n파일 종류와 개수: ${files}\n원하는 일정: ${deadline || "협의 가능"}\n\n원하는 결과물:\n\n샘플 자료는 개인정보를 제거한 뒤 별도로 전달하겠습니다.`;
  return <form className="au-inquiry" onSubmit={event => { event.preventDefault(); window.location.href = `mailto:allweek@naver.com?subject=${encodeURIComponent(`[툴스랩] ${service} 문의`)}&body=${encodeURIComponent(body)}`; setNotice("메일 앱에서 내용을 확인한 뒤 직접 보내주세요. 메일 앱이 없다면 문의 내용을 복사해 allweek@naver.com으로 보내주세요."); }}>
    <label htmlFor="au-service">문의 유형<select id="au-service" value={service} onChange={e => setService(e.target.value)}><option>49,000원 진단·작동 샘플</option><option>190,000원부터 맞춤 제작</option><option>어떤 상품이 맞는지 상담</option></select></label>
