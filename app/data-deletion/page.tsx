    ...
    <LegalShell eyebrow="TREND THREADS DATA DELETION" title="Account and Data Deletion" summary="English is the primary deletion-guide language. Complete Korean instructions are provided below.">
      ...
      <div lang="en">
        <PolicySection title="Disconnect the optional Threads account">
          ...
          <p>Disconnect deletes the encrypted Threads access token, app-scoped account binding, automatic-ranking preference, pending jobs, encrypted baselines and completed snapshots, and any pending OAuth state from the service. It does not delete the user&apos;s Threads account or Threads content. Content-free request lifecycle receipts expire within 24 hours.</p>
          <p>Automatic baselines and completed snapshots are owner-bound AES-256-GCM ciphertext, become stale after 75 minutes, and are deleted no later than two hours after observation. Optional precise manual result content and observations are transient and are not retained in the server database or device cache.</p>
        </PolicySection>
        ...
        <PolicySection title="Delete the Trend Threads account and server data in the app">
          ...
          <p>Account deletion removes the Supabase authentication account, server preferences and consent records, Threads connection, encrypted access token, account binding, automatic-ranking preference, pending jobs, encrypted baselines and completed snapshots, pending OAuth state, and lifecycle receipts owned by that user through the account-deletion process.</p>
        </PolicySection>
        ...
        <PolicySection title="Delete local activity on this device">
          ...
          <p>Prepared post-result content, usernames, permalinks, exact result counts, and observations are never written to the device cache. Delete local preferences separately if they must be removed together with the server account.</p>
        </PolicySection>
        ...
        <PolicySection title="Content on Threads">
          <p>Trend Threads displays safety-filtered post text, usernames, publication time, and exact official permalinks from a short-lived encrypted owner-specific snapshot. Snapshots become stale after 75 minutes and are deleted no later than two hours after observation. Results are not stored in device cache, analytics, advertising systems, or any public or cross-user aggregate. Media and exact result counts are not stored in snapshots, and media is not displayed, copied, or downloaded.</p>
      ...
      <div lang="ko">
        <PolicySection title="선택적 Threads 계정 연결 해제">
          ...
          <p>연결 해제는 서비스에 저장된 암호화 Threads 액세스 토큰, 앱 범위 계정 연결, 자동 순위 설정, 대기 작업, 암호화 baseline·완성 snapshot과 대기 중 OAuth state를 삭제합니다. 사용자의 Threads 계정이나 Threads 콘텐츠는 삭제하지 않습니다. 콘텐츠가 없는 요청 lifecycle 기록은 24시간 이내 만료됩니다.</p>
          <p>자동 baseline과 완성 snapshot은 사용자별 AES-256-GCM 암호문으로만 단기 저장하고 75분 후 오래된 상태가 되며 관찰 후 최대 2시간 안에 삭제합니다. 선택적 정밀 수동 측정의 결과 콘텐츠와 관찰은 일시 처리하며 서버 DB나 기기 cache에 보관하지 않습니다.</p>
        </PolicySection>
        ...
        <PolicySection title="앱에서 Trend Threads 계정과 서버 데이터 삭제">
          ...
          <p>계정 삭제는 계정 삭제 절차를 통해 Supabase 인증 계정, 서버 환경설정과 동의 기록, Threads 연결, 암호화 액세스 토큰, 계정 binding, 자동 순위 설정, 대기 작업, 암호화 baseline·완성 snapshot, 대기 중 OAuth state와 해당 사용자가 소유한 lifecycle 기록을 제거합니다.</p>
        </PolicySection>
        ...
        <PolicySection title="이 기기의 로컬 활동 삭제">
          ...
          <p>준비된 게시물 결과 콘텐츠, 사용자 이름, permalink, 정확한 결과 수와 관찰은 기기 cache에 기록하지 않습니다. 서버 계정과 함께 환경설정도 지워야 하는 경우 로컬 활동 삭제를 별도로 실행하세요.</p>
        </PolicySection>
        ...
        <PolicySection title="Threads의 콘텐츠">
          <p>Trend Threads는 단기 암호화한 사용자별 snapshot에서 안전 필터를 통과한 게시물 본문, 사용자 이름, 게시 시각과 정확한 공식 permalink를 표시합니다. snapshot은 75분 후 오래된 상태가 되고 관찰 후 최대 2시간 안에 삭제합니다. 결과는 기기 cache, analytics, 광고 시스템 또는 공개·사용자 간 집계에 저장하지 않습니다. 미디어와 정확한 결과 수는 snapshot에 저장하지 않고 미디어를 표시·복제·다운로드하지 않습니다.</p>
